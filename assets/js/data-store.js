(function () {
  ...
  function mapStudentHealthFromRemote(item) {
    ...
    return {
      ...
      conditionName: item.condition_name || item.conditionName || "",
  ...
  function mapStudentHealthToRemote(item) {
    ...
    return {
      ...
      category: item.category || "other",
      condition_name: item.conditionName || item.condition_name || "",
      observation: item.observation || "",
  ...
  function mapActivityFromRemote(item) {
    ...
    return {
      ...
      status: item.status || "draft",
      targetTurmas: normalizeArray(item.target_turmas || item.targetTurmas).filter(Boolean),
      targetStudentIds: normalizeArray(item.target_student_ids || item.targetStudentIds).filter(Boolean),
  ...
  function mapActivityToRemote(item) {
    ...
    return {
      ...
      status: item.status || "draft",
      target_turmas: normalizeArray(item.targetTurmas || item.target_turmas).filter(Boolean),
      target_student_ids: normalizeArray(item.targetStudentIds || item.target_student_ids).filter(Boolean),
      attachments: normalizeArray(item.attachments).filter(Boolean),
